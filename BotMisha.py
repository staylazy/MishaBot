...
for i in range(value // 6):
    data = api.photos.get(owner_id = user_id, album_id = album, rev = 0, offset = offset_pic, count = 6, v = 5.77)
    offset_pic += 6
    ...
    year = int(list_date[4]) * 1000 + int(list_date[5]) * 100 + int(list_date[6]) * 10 + int(list_date[7])
    if month == 12 and day == 31:
        ...
        new_month(month, list_date)
    elif month == 2 and day == 29 and not (year % 4 != 0 or (year % 100 == 0 and year % 400 != 0)):
        new_month(month, list_date)
    else:
        day += 1
    ...
    for item in data['items']:
        print('id картинки: ', item.get('id'), "|", "Дата и время репоста: ", 
            time.strftime('%d-%m-%Y %H:%M:%S', time.localtime(date)), "|", "Attachments: ", mes) 
        picture_id = item.get('id')
        picture = 'photo' + str(user_id) + '_'
        picture += str(picture_id)
        api.wall.post(owner_id = pabl_id, attachments = picture, from_group = 1, message = str(mes) + '/10850', publish_date = date, v = 5.77)
        date += frequency
        mes = int(mes) + 1
        mes = str(mes)
os.system("pause")